...
(function ($) {
    ...
    $.fn.iqDropdown2 = function (options) {
        this.each(function () {
            ...
            const $buttons = $menu.find('div.iqdropdown-button');
        ...
        `);
                ...
                $clearButton.click((event, $this) => {
                    console.log(event.target.closest('.iqdropdown'))
                    if(event.target.closest('.iqdropdown')){
                        itemCount[id] = 0;
                        totalItems = 0;
                        $counter.html(itemCount[id]);
                        updateDisplay();
                ...
                $clearButton.click(event => event.stopPropagation());
...
$(document).ready(() => {
    let deleteButton = $('.iqdropdown-button__delete');


    $('.iqdropdown.guest').iqDropdown2({
        selectionText:'Guest',  
        textPlural:'Guests',
        clearButton: deleteButton,
        setSelectionText: (itemCount, totalItems) => {
            if (totalItems === 0) {return 'Сколько гостей'}
            else if (totalItems === 1) {return totalItems + ' гость'}
            else if (totalItems < 5)  {return totalItems + ' гостя'}
            else if (totalItems => 5) {return totalItems + ' гостей'}
